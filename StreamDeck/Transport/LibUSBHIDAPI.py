...
class LibUSBHIDAPI(Transport):
    ...
    class Library():
        ...
        def _get_addon_hidapi_path(self):
            """Return the path to the addon's bundled hidapi.libs directory, if present."""
            addon_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
            hidapi_libs_dir = os.path.join(addon_dir, 'hidapi.libs')
            if os.path.isdir(hidapi_libs_dir):
                return hidapi_libs_dir
            return None
        ...
        def _load_hidapi_library(self, library_search_list):
            ...
            # Build an extended search list that includes absolute paths from the
            # addon's bundled hidapi.libs/ directory (used on NixOS where system
            # library paths are not in the standard ldconfig search paths).
            addon_hidapi_dir = self._get_addon_hidapi_path()
            extended_search_list = []
            if addon_hidapi_dir:
                for lib_name in library_search_list:
                    candidate = os.path.join(addon_hidapi_dir, lib_name)
                    if os.path.exists(candidate):
                        extended_search_list.append(candidate)
            extended_search_list.extend(library_search_list)

            for lib_name in extended_search_list:
